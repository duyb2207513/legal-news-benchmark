...
retrieve(question, mode)
   ...
   │
   ▼ (tuỳ chọn) rerank() — lọc theo coverage từ khoá, sort lại
   ▼ sort ưu tiên "Còn hiệu lực", cắt còn max_components
   │
   ▼
seeds (list[dict] Component)
   │
   ├─ mode="graphrag": expand_graph(seeds) → build_context_graph()
...
#### `fusion.py` — Hợp nhất kết quả (RRF)

- **Hàm:** `merge_search_results(*result_lists, k=60) -> list[dict]`
- **Input:** nhiều list kết quả (mỗi list đã sort theo score giảm dần) — ví dụ `vector_search()` và `bm25_search()`.
- **Output:** 1 danh sách hợp nhất, mỗi dòng có thêm `rrf_score`, `original_score`, `best_rank`, đã loại trùng theo `(norm_id, citation)` và sort ưu tiên (còn hiệu lực trước, rrf_score giảm dần, best_rank tăng dần).
- **Ý nghĩa:** Reciprocal Rank Fusion — công thức `rrf(row) = Σ 1/(k + rank_i)` qua các nguồn chứa row đó. Dùng RANK thay vì raw score vì cosine similarity (vector) và BM25F score không cùng thang đo, so sánh trực tiếp sẽ thiên vị BM25 (score luôn lớn hơn "giả tạo"). Nguồn đầu tiên (quy ước là vector search) được thưởng hệ số 1.2 vì có precision cao hơn cho câu hỏi tự nhiên.
...
- **Ý nghĩa:** RRF chỉ xếp theo rank giữa 2 nguồn, không đánh giá lại độ liên quan ngữ nghĩa thật. Bản cũ dùng self-referential BM25 normalization (chuẩn hoá theo max trong chính tập seeds) nên luôn đẩy 1 candidate lên ~10/10 dù nó lạc đề — không bao giờ lọc sạch được cả tập sai. Bản hiện tại dùng **coverage tuyệt đối**: `coverage = |query_terms ∩ doc_terms| / |query_terms|` (đã loại stopword tiếng Việt), không phụ thuộc các candidate khác — nếu tài liệu không chứa từ khoá nào của câu hỏi, coverage = 0 và bị loại bất kể so với các candidate khác thế nào.
...
- **Output:** 1 chuỗi text (context) ghép các đoạn, phân cách bằng `---`, để đưa vào `RAG_PROMPT`.
- **Ý nghĩa:** Cả 2 đều sort ưu tiên văn bản "Còn hiệu lực" lên đầu. `build_context_graph` bổ sung thêm dòng "Tác động ra ngoài" / "Bị tác động" khi Component từng sửa đổi hoặc bị sửa đổi bởi văn bản khác — giúp trả lời được câu hỏi kiểu "quy định này còn hiệu lực không, bị thay bởi văn bản nào". Nếu `n` (Norm) là `None`, vẫn giữ Component trong context nhưng bỏ phần header Norm thay vì loại bỏ hẳn.
...
- **`retrieve(question, mode, top_k=TOP_K, max_components=MAX_COMPONENTS, use_rerank=USE_RERANK, client=None, embedding=None, keywords=None) -> list[dict]`**
  ...
  - **Output:** tối đa `max_components` Component liên quan nhất (đã qua rerank nếu bật, và sort ưu tiên "Còn hiệu lực").
  - **Ý nghĩa:** Điều phối theo mode — gọi `vector_search`/`bm25_search`/`merge_search_results` tương ứng; với `hybrid`/`graphrag`, nếu cả embedding và keywords đều chưa có thì tính song song bằng `ThreadPoolExecutor` (2 việc độc lập, tiết kiệm latency).
- **`run_pipeline(question, mode, top_k, max_components, max_chars, max_tokens, client=None, embedding=None, keywords=None) -> dict`**
  - **Input:** giống `retrieve` + `max_chars` (giới hạn ký tự/đoạn) và `max_tokens` (giới hạn token prompt).
  - **Output:** dict gồm `question, mode, answer, context, retrieved, prompt_tokens, latency_sec, retrieved_citations`.
  - **Ý nghĩa:** Hàm end-to-end — `retrieve()` → build context (flat hoặc graph tuỳ mode) → cắt bớt context nếu prompt vượt `max_tokens` → sinh câu trả lời bằng `answer()`. Chỉ mở **1 Neo4jClient** dùng chung cho cả `vector_search` lẫn `graph_expand` trong 1 lần gọi, tránh mở/đóng driver lặp lại; nếu không truyền `client` từ ngoài, tự mở/đóng riêng (phù hợp gọi lẻ). Đây là hàm được cả app thật và `benchmark/run_benchmark.py` gọi.
...
eval_set.jsonl  ──► load_eval_set()  ──► list[{question, category?}]
                          ...
                          │   4 mode chạy TUẦN TỰ bên trong 1 câu hỏi)  │
                          └───────────────────┬───────────────────┘
                                              ▼
...
- **Hàm:**
  - `_run_one_question(item, modes, include_reason=False) -> list[dict]`: chạy tuần tự tất cả mode (`bm25/vector/hybrid/graphrag`) cho 1 câu hỏi, mỗi mode gọi `run_pipeline()` rồi `score_with_deepeval()`, trả 1 dòng/mode gồm điểm số + `context_len_chars`, `prompt_tokens`, `latency_sec` + các cột `_*_reason` (debug).
  - `run_benchmark(eval_set, modes=MODES, max_workers=4, include_reason=False) -> pd.DataFrame`: chạy `_run_one_question` cho toàn bộ eval set, **song song hoá theo câu hỏi** (mỗi câu hỏi 1 thread, nhưng 4 mode bên trong 1 câu hỏi vẫn chạy tuần tự — tránh quá tải rate limit Vertex AI/Neo4j). Trả DataFrame 1 dòng/(câu hỏi, mode).
  - `summarize(df_results) -> pd.DataFrame`: trung bình các metric số theo `mode` — bảng chính so sánh 4 mode.
  - `summarize_by_category(df_results) -> pd.DataFrame | None`: trung bình theo `category x mode` (chỉ có nếu EVAL_SET có cột `category`) — giúp thấy GraphRAG nổi bật ở nhóm câu hỏi cần suy luận nhiều văn bản.
  - `main()`: parse CLI args (`--eval-set`, `--modes`, `--out-dir`, `--max-workers`, `--include-reason`), chạy benchmark, in bảng tổng hợp, lưu 2 file CSV (`benchmark_results.csv` chi tiết từng dòng, `benchmark_summary.csv` trung bình theo mode) vào `benchmark/results/`.
- **Input/Output tổng thể:** Input là 1 file jsonl EVAL_SET (đường dẫn qua `--eval-set`). Output là 2 file CSV trong thư mục kết quả + bảng in ra console.
- **Ý nghĩa:** Đây là script so sánh khách quan 4 chiến lược retrieval trên cùng 1 bộ câu hỏi, dùng DeepEval (không cần ground truth) làm giám khảo tự động. `DEFAULT_MAX_WORKERS = 4` được chọn làm mức an toàn cho quota Vertex AI mặc định, tránh dính rate limit khi chạy song song quá nhiều.
...
```bash
python -m benchmark.run_benchmark --eval-set data/questions.jsonl --modes bm25 vector hybrid graphrag --max-workers 4
```
