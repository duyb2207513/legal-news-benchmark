...
Hệ thống RAG (Retrieval-Augmented Generation) trả lời câu hỏi pháp luật Việt Nam, xây dựng trên nền **Legal Knowledge Graph** (Neo4j) và so sánh 4 chiến lược retrieval: `bm25`, `vector`, `hybrid`, `graphrag`.
...
│   ├── graph_expand.py, context_builder.py, prompts.py
...
### 4.4 Chạy benchmark so sánh 4 mode
...
Xem thêm cờ `--include-reason` để DeepEval sinh giải thích cho từng metric (tốn thêm 1 lần gọi LLM/metric/câu hỏi).
